...
public abstract class ServoMotorSubsystem extends Subsystem {
    ...
    protected ServoMotorSubsystem(final ServoMotorSubsystemConstants constants) {
        ...
        mForwardSoftLimitRotations = (((mConstants.kMaxUnitsLimit - mConstants.kHomePosition)
                * mConstants.kRotationsPerUnitDistance) - mConstants.kSoftLimitDeadband);
        ...
        mMasterConfig.MotorOutput.Inverted = (mConstants.kMasterConstants.counterClockwisePositive
        ...
        for (int i = 0; i < mSlaves.length; ++i) {
            mSlaves[i] = TalonFXFactory.createPermanentSlaveTalon(mConstants.kSlaveConstants[i].id,
                    mConstants.kMasterConstants.id, false);
            ...
            slaveConfig.MotorOutput.Inverted = (mConstants.kMasterConstants.counterClockwisePositive
                    ? InvertedValue.CounterClockwise_Positive
                    : InvertedValue.Clockwise_Positive);
    ...
    public synchronized void readPeriodicInputs() {
        ...
        if (Util.epsilonEquals(newVelocity, mConstants.kCruiseVelocity, Math.max(1, mConstants.kDeadband)) || Util
                .epsilonEquals(newVelocity, mPeriodicIO.active_trajectory_velocity,
            ...
            mPeriodicIO.active_trajectory_acceleration = Math
                    .signum(newVelocity - mPeriodicIO.active_trajectory_velocity) * mConstants.kAcceleration;
        }
    ...
    public synchronized void writePeriodicOutputs() {
        if (mControlState == ControlState.MOTION_MAGIC) {
            mMaster.setControl(new MotionMagicVoltage(mPeriodicIO.demand).withFeedForward(mPeriodicIO.feedforward)
                    .withSlot(kMotionMagicSlot));
        } else if (mControlState == ControlState.POSITION_PID || mControlState == ControlState.MOTION_PROFILING) {
            mMaster.setControl(new PositionDutyCycle(mPeriodicIO.demand).withFeedForward(mPeriodicIO.feedforward)
                    .withSlot(kPositionPIDSlot));
        } else {
        ...
        mEnabledLooper.register(new Loop() {
            ...
            public void onLoop(double timestamp) {
                if (mPeriodicIO.reset_occured) {
                    ...
                    PhoenixProUtil.checkErrorAndRetry(() -> mMasterOutputVoltageSignal.setUpdateFrequency(200, 0.05));
                    PhoenixProUtil
                            .checkErrorAndRetry(() -> mMasterOutputPercentageSignal.setUpdateFrequency(200, 0.05));
    ...
    public synchronized boolean hasFinishedTrajectory() {
        return Util.epsilonEquals(mPeriodicIO.active_trajectory_position, getSetpoint(),
                Math.max(1, mConstants.kDeadband));
    }
    ...
    public synchronized double getSetpoint() {
        return (mControlState == ControlState.MOTION_MAGIC ||
                ...
                mControlState == ControlState.MOTION_PROFILING) ? rotationsToHomedUnits(mPeriodicIO.demand)
                        : Double.NaN;
    }
    ...
    public synchronized void setSetpointMotionProfiling(IMotionProfileGoal goal, double feedforward_v) {
        if (mControlState != ControlState.MOTION_PROFILING) {
            ...
            mMotionStateSetpoint = new MotionState(mPeriodicIO.timestamp, mPeriodicIO.position_units,
                    rotationsToUnits(mPeriodicIO.velocity_rps), 0.0);
            mSetpointGenerator.reset();
        }
        Setpoint setpoint = mSetpointGenerator.getSetpoint(mMotionProfileConstraints, goal, mMotionStateSetpoint,
                mPeriodicIO.timestamp + mConstants.kLooperDt);
        mPeriodicIO.demand = constrainRotations(homeAwareUnitsToRotations(setpoint.motion_state.pos()));
        mPeriodicIO.feedforward = mConstants.kPositionKf
                + (unitsToRotations(feedforward_v + setpoint.motion_state.vel()) * mConstants.kKf +
    ...
    public synchronized void outputTelemetry(boolean disabled) {
        if (disabled) {
            SmartDashboard.putNumber(mConstants.kName + ": Position (units)",
                    rotationsToHomedUnits(mPeriodicIO.position_rots));
        ...
        // SmartDashboard.putNumber(mConstants.kName + " velocity (rps)",
        // mMasterVelocitySignal.asSupplier().get());
        // SmartDashboard.putNumber(mConstants.kName + " position error(units) ",
        // mMasterClosedLoopError.asSupplier().get() /
        // mConstants.kRotationsPerUnitDistance);
        // SmartDashboard.putNumber(mConstants.kName + " Closed Loop Error",
        // mMasterClosedLoopError.asSupplier().get());
        // SmartDashboard.putNumber(mConstants.kName + " Closed Loop Output",
        // mMasterClosedLoopOutputSignal.asSupplier().get());
        // SmartDashboard.putNumber(mConstants.kName + " Closed Loop Reference",
        // mMasterClosedLoopReferenceSignal.asSupplier().get());
        // SmartDashboard.putNumber(mConstants.kName + " Closed Loop Reference Slope",
        // mMasterClosedLoopReferenceSlopeSignal.asSupplier().get());
        // SmartDashboard.putNumber(mConstants.kName + " Closed Loop Velocity Error",
        // mMasterClosedLoopReferenceSlopeSignal.asSupplier().get() -
        // mMasterVelocitySignal.asSupplier().get());
    }
