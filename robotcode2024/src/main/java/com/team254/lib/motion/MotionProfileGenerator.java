 ...
 * a given MotionState to a given
...
public class MotionProfileGenerator {
    ...
    protected static MotionProfile generateFlippedProfile(MotionProfileConstraints constraints,
        ...
        MotionProfile profile = generateProfileImpl(constraints, goal_state.flipped(), prev_state.flipped(),
                !origFlipped);
     ...
     * @return A motion profile from prev_state to goal_state that satisfies
     *         constraints.
     */
    public synchronized static MotionProfile generateProfile(MotionProfileConstraints constraints,
     ...
     *
     * @param constraints The constraints to use. Note that max_vel must ==
     *                    -min_vel.
     * @param goal_state  The goal to use. If goal_vel is outside the constraints,
     *                    it is clamped.
     * @param prev_state  The initial state to use.
     * @return A motion profile from prev_state to goal_state that satisfies
     *         constraints.
    ...
    public synchronized static MotionProfile generateProfile(MotionProfileConstraints constraints,
            ...
            MotionState prev_state) {
        // The trick we use to solve this problem simply requires symmetrical velocity
        // constraints.
        ...
        if (goal_state.max_vel() >= constraints.max_vel() || goal_state.max_vel() <= constraints.min_vel()) {
            // Reaching the goal exactly is impossible, so just move towards the goal as
            // quickly as possible.
            return generateProfile(constraints,
                    new MotionProfileGoal(Math.signum(goal_state.max_vel()) * Double.MAX_VALUE, goal_state.max_vel()),
                    prev_state);
        }

        // Turn the linear time-varying goal into a stationary goal by superimposing the
        // (opposite of) goal state motion
        // onto the initial state and all constraints. Then add the goal state motion
        // back to the result.
        MotionState superposedPrevState = new MotionState(prev_state.t(), prev_state.pos(),
                prev_state.vel() - goal_state.max_vel(), prev_state.acc());
        MotionProfileGoal superposedGoal = goal_state.toMotionProfileGoal(prev_state.t());
        MotionProfileConstraints superposedConstraints = new MotionProfileConstraints(
                constraints.max_vel() - goal_state.max_vel(),
        ...
        // As a result of the shift, individual segments may now (a) be able to be
        // combined, (b) have to be split in order
        ...
        for (var segment : profile.segments()) {
            if (segment.start().vel() * segment.end().vel() >= 0.0) {
                ...
                // Velocity sign changes midway through segment. Figure out where and make two
                // new segments around it.
    ...
    protected static MotionState shiftByVelocity(MotionState state, double velocity_shift, double start_time) {
        return new MotionState(state.t(), state.pos() + velocity_shift * (state.t() - start_time),
                state.vel() + velocity_shift, state.acc());
    }
    ...
    protected static MotionProfile generateProfileImpl(MotionProfileConstraints constraints,
        ...
        if (delta_pos < 0.0 || (delta_pos == 0.0 && prev_state.vel() < 0.0)) {
            // For simplicity, we always assume the goal requires positive movement. If
            // negative, we flip to solve, then
            // flip the solution.
        ...
        // If our velocity is headed away from the goal, the first thing we need to do
        // is to stop.
        ...
        if (min_abs_vel_at_goal_sqr > 0.0
            ...
            // completion_behavior to see what we should
            // do.
            if (goal_state.completion_behavior() == CompletionBehavior.VIOLATE_MAX_ABS_VEL) {
                ...
                // We are going to overshoot the goal, so the first thing we need to do is come
                // to a stop.
                final double stopping_time = Math.abs(start_state.vel() / constraints.max_abs_acc());
                profile.appendControl(-constraints.max_abs_acc(), stopping_time);
        ...
        // Invariant from this point forward: We can achieve goal_vel at goal_state.pos
        // exactly using no more than +/-
        // max_acc.

        // What is the maximum velocity we can reach (Vmax)? This is the intersection of
        // two curves: one accelerating
        // towards the goal from profile.finalState(), the other coming from the goal at
        // max vel (in reverse). If Vmax
