 ...
 * The controller uses feedforward on
 * acceleration, velocity, and position; proportional feedback on velocity and
 * position; and integral feedback on
 * position.
 */
public class ProfileFollower {
     ...
     * @param kv   The proportional gain on velocity error (or derivative gain on
     *             position error).
     * @param kffv The feedforward gain on velocity. Should be 1.0 if the units of
     *             the profile match the units of the
     *             output.
    ...
    /**
     * @return The current goal (null if no goal has been set since the latest call
     *         to reset()).
     */
    ...
    /**
     * Reset just the setpoint. This means that the latest_state provided to
     * update() will be used rather than feeding
     * forward the previous setpoint the next time update() is called. This almost
     * always forces a MotionProfile update,
     ...
     *
     * @param latest_state The latest *actual* state, used only for feedback
     *                     purposes (unless this is the first iteration or
     *                     reset()/resetSetpoint() was just called, in which case
     *                     this is the new start state for the profile).
     * @param t            The timestamp for which the setpoint is desired.
     * @return An output that reflects the control output to apply to achieve the
     *         new setpoint.
     */
    ...
    /**
     * We are finished the profile when the final setpoint has been generated. Note
     * that this does not check whether we
    ...
    /**
     * We are on target if our actual state achieves the goal (where the definition
     * of achievement depends on the goal's
     * completion behavior).
     *
