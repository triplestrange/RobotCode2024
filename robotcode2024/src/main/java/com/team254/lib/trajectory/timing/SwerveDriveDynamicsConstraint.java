...
public class SwerveDriveDynamicsConstraint<S extends IPose2d<S> & ICourse2d<S> & ICurvature<S> & IHeadingRate<S>>
    ...
    public double getMaxVelocity(S state) {
        ...
        Rotation2d course_local = state.getPose().getRotation().inverse()
                .rotateBy(course.isPresent() ? course.get() : Rotation2d.kIdentity);
