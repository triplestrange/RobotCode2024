...
public class TrajectoryUtil {
    ...
    public static <S extends IPose2d<S>> Trajectory<TimedState<S>> mirrorTimed(
            final Trajectory<TimedState<S>> trajectory) { // todo fix
        List<TimedState<S>> waypoints = new ArrayList<>(trajectory.length());
        for (int i = 0; i < trajectory.length(); ++i) {
            TimedState<S> timed_state = trajectory.getPoint(i).state();
            waypoints.add(new TimedState<>(timed_state.state().mirror(), timed_state.t(), timed_state.velocity(),
    ...
    public static Trajectory<Pose2dWithMotion> trajectoryFromWaypointsAndHeadings(final List<Pose2d> waypoints,
    ...
    public static Trajectory<Pose2dWithMotion> trajectoryFromSplines(final List<? extends PoseSpline> splines,
