 ...
 * you can set your set
 * your center of rotation in a corner of the robot to perform special evasion
 * maneuvers.
 *
 * <p>
...
public class SwerveDriveKinematics {
     ...
     * of wheel locations
     * as Translation2ds. The order in which you pass in the wheel locations is the
     * same order that
     * you will receive the module states when performing inverse kinematics. It is
     * also expected that
     * you pass in the module states in the same order when calling the forward
     * kinematics methods.
     *
     * @param wheelsMeters The locations of the wheels relative to the physical
     *                     center of the robot.
     */
     ...
     * given module states.
     * This method is often used for odometry -- determining the robot's position on
     * the field using
     * data from the real-world speed and angle of each module on the robot.
     *
     * @param wheelStates The state of the modules (as a SwerveModuleState type) as
     *                    measured from
     *                    respective encoders and gyros. The order of the swerve
    ...
    public ChassisSpeeds toChassisSpeedWheelConstraints(SwerveModuleState... wheelStates) {
        ...
        for (int i = 0; i < m_numModules; i++) {
            ...
            var beta = module.angle.rotateBy(
    ...
    public static ChassisSpeeds compensateForSkew(ChassisSpeeds speeds) {
        if (Util.epsilonEquals(speeds.omegaRadiansPerSecond, 0, 1e-2))
            return speeds;

        double kFactor = 0.1;
        double magnitude = Math.hypot(speeds.vxMetersPerSecond, speeds.vyMetersPerSecond) * speeds.omegaRadiansPerSecond
                * kFactor;
